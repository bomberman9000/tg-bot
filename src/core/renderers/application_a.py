"""
Рендер текста заявки A (Клиент ⇄ Экспедитор) из Jinja2-шаблона.
Все реквизиты — из parties (client, forwarder); при отсутствии поля — «—».
"""
...
def _deal_context(cargo: Cargo | object, app_id: int | None = None) -> dict:
    ...
    return {
        ...
        "carrier_price": price,
        "price": price,
        "payment_vat_text": getattr(cargo, "payment_vat_text", None) or "—",
...
def render_application_a(deal: Cargo | dict, parties: dict, *, app_id: int | None = None) -> str:
    """
    parties: {"client": payload, "forwarder": payload}
    ...
    template = env.get_template("application_a.txt")
    ...
    client = parties.get("client") or _empty_party()
